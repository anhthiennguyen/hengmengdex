...
export default function LobbyPage({ mode, lobbyCode, onLeave }) {
  const { snapshot, engine } = useLobbyEngine(mode, lobbyCode);
    ...
    <Shell onLeave={onLeave}>
      <div className="mb-4 flex items-center justify-between gap-2 rounded-lg bg-zinc-50 px-3 py-2">
        ...
        <button
          type="button"
          onClick={copyLink}
          className="flex shrink-0 items-center gap-1 rounded-md bg-royal-600 px-2.5 py-1 text-xs font-bold text-white hover:bg-royal-700"
        >
...
function Shell({ children, onLeave }) {
  return (
    <div className="mx-auto max-w-md px-4 py-10">
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-lg font-extrabold text-zinc-900">Battle Lobby</h1>
        <button
          type="button"
          onClick={onLeave}
